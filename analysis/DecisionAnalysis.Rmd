...
narps_df = read_csv(paste(basedir,'metadata/all_metadata.csv',sep='/'))
...
anova_hyp<- anova(m_hyp_full,m_hyp_nohyp,test='Chisq')
print(anova_hyp)
...
anova_smoothing <- anova(m_hyp_full,m_hyp_nosmooth,test='Chisq')
...
anova_fmriprep <- anova(m_hyp_full,m_hyp_noprep,test='Chisq')
...
anova_package <- anova(m_hyp_full,m_hyp_nopackage,test='Chisq')
print(anova_package)
...
anova_testing <- anova(m_hyp_full,m_hyp_notesting,test='Chisq')
print(anova_testing)
...
ggplot(decisionSummary_fmriprep,aes(varnum,meanAccept,color=used_fmriprep_data, group=used_fmriprep_data)) + geom_line()
...
patterndist_df = read_csv(paste(basedir,'metadata/median_pattern_distance.csv',sep='/'),skip=0)
...
df_SuppTable5[1,2:4] = c(anova_hyp$`Chisq`[2],
                         anova_hyp$`Pr(>Chisq)`[2],
                         delta_r2_hyp[2,1]
                         )
df_SuppTable5[2,2:4] = c(anova_smoothing$`Chisq`[2],
                         anova_smoothing$`Pr(>Chisq)`[2],
                         delta_r2_smoothing[2,1]
                         )
df_SuppTable5[3,2:4] = c(anova_fmriprep$`Chisq`[2],
                         anova_fmriprep$`Pr(>Chisq)`[2],
                         delta_r2_fmriprep[2,1]
                         )
df_SuppTable5[4,2:4] = c(anova_package$`Chisq`[2],
                         anova_package$`Pr(>Chisq)`[2],
                         delta_r2_package[2,1]
                         )
df_SuppTable5[5,2:4] = c(anova_testing$`Chisq`[2],
                         anova_testing$`Pr(>Chisq)`[2],
                         delta_r2_testing[2,1]
                         )

print(df_SuppTable5)
xtable(df_SuppTable5)
```
