...
lower_p <- apply(Out$t<0,2,mean)
upper_p <- apply(Out$t>0,2,mean)
pvals <- rbind(upper_p,lower_p)
boot_pval <- apply(pvals,2,min)*2

# loop over parametric regressors and get percentile CI
for (i in 1:3){
  ci_raw <- boot::boot.ci(boot.out = Out, type = c("perc"), index = i)
  percentile_np_ci[i,] <- c(ci_raw$percent[4],npboot_means[i],ci_raw$percent[5],boot_pval[i])
}
...
for (i in 4:9){
  ...
  percentile_np_ci[i,] <- c(ci_raw$percent[4],npboot_means[i],ci_raw$percent[5],lower_p[i])
}
...
summary_df['Bootstrap P-value'] = c(NA,
                              percentile_np_ci['fwhm','boot_pval'],
                              percentile_np_ci['used_fmriprep_data','boot_pval'],
                              NA,
                              NA,
                              percentile_np_ci['movement_modeling', 'boot_pval'])
summary_df['Model selection probability (BIC)'] = c(percentile_np_ci['Hypothesis (deltaBIC)','boot_pval'],
                              NA,
                              NA,
                              percentile_np_ci['package (deltaBIC)','boot_pval'],
                              percentile_np_ci['testing (deltaBIC)','boot_pval'],
                              NA)
summary_df['Model selection probability (AIC)'] = c(percentile_np_ci['Hypothesis (deltaAIC)','boot_pval'],
                              NA,
                              NA,
                              percentile_np_ci['package (deltaAIC)','boot_pval'],
                              percentile_np_ci['testing (deltaAIC)','boot_pval'],
                              NA)
