...
for (i in 4:6){
  ci_raw <- boot::boot.ci(boot.out = Out, type = c("perc"), index = i)
  # use upper p because we want to know how often full model
  # is better than reduced model
  percentile_np_ci[i,] <- c(ci_raw$percent[4],npboot_means[i],ci_raw$percent[5],upper_p[i])
}
...
summary_df['Model selection probability (using BIC)'] = c(percentile_np_ci['Hypothesis (deltaBIC)','boot_pval'],
                              NA,
                              NA,
